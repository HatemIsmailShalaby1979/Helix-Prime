...
def compute_coach_dashboard(
    ...
    today_sessions = [s for s in sessions if s.coach_id == coach_id
    ...
    return {
        ...
        "today_sessions": [
            {"session_id": s.session_id, "date": s.date, "start": s.start,
             "end": s.end, "program_id": s.program_id, "roster_size": len(s.roster)}
...
def render_coach_dashboard(st, ctx: ConnectorContext,
                           connectors: Dict[str, Any], as_of: str,
                           coach_id: str, date: Optional[str] = None) -> None:
    ...
    if board["today_sessions"]:
        ...
        for sid, row in board["today_attendance"].items():
            st.progress(row["attendance_rate"],
    ...
    for col, key, fmt in ((col1, "session_adherence", "{:.0%}"),
        ...
        col.metric(f"{met} {key.replace('_', ' ').title()}", value,
                   delta=f"target {entry['target']:.0%}" if isinstance(entry["target"], float) else f"target {entry['target']}")
