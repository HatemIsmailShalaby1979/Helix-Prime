...
def build_synthetic_academy(tenant_id: str, client_id: str, as_of: str) -> dict:
    ...
    coaches = [
        Coach(f"coach-{i}", name, tenant_id, client_id, _src("HR", f"coach-{i}", as_of))
    ...
    for day_idx, date in enumerate(_SESSION_DATES):
        for prog_idx, program_id in enumerate(("prog-u12", "prog-u15")):
            ...
            for athlete_id in roster:
                ...
                attends = (
                    (rng.random() < 0.85)
                    if athlete_id not in ("ath-01", "ath-02")
                    else (day_idx < 3)
                )
                if not attends:
                    continue
    ...
    fee_payments = [
        ...
        FeePayment(
            ...
            "2026-09-01",
            None,
            "outstanding reminder sent",
            tenant_id,
            client_id,
            _src("Billing", "pay-003", as_of),
        ),
    ]
    ...
    enrollment_records = [
        EnrollmentRecord(
            ...
            "parent called, wants Saturday trial",
            tenant_id,
            client_id,
