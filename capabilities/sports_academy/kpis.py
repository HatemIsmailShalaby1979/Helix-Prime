...
def attendance_rate(
    athletes: Sequence[Athlete], sessions: Sequence[Session], checkins: Sequence[CheckIn]
) -> float:
    if not sessions:
        return 0.0
...
def monthly_recurring_revenue(athletes: Sequence[Athlete], programs: Sequence[Any]) -> float:
    ...
    total = sum(
        fee_by_program.get(a.program_id, 0.0) for a in athletes if a.enrollment_status == "active"
    )
...
def compute_academy_metrics(
    *,
    ...
    fee_payments: Optional[Sequence[FeePayment]] = None,
) -> Dict[str, Any]:
    """The 5 owner-dashboard numbers + targets, from pack data."""
    return {
        "attendance_rate": _with_target(
            attendance_rate(athletes, sessions, checkins),
            "attendance_rate",
            ACADEMY_KPI_TARGETS,
            direction="higher_is_better",
        ),
        "churn_rate": _with_target(
            churn_rate(athletes), "churn_rate", ACADEMY_KPI_TARGETS, direction="lower_is_better"
        ),
        "facility_utilization": _with_target(
            facility_utilization(facility_slots),
            "facility_utilization",
            ACADEMY_KPI_TARGETS,
            direction="higher_is_better",
        ),
        "mrr": _with_target(
            monthly_recurring_revenue(athletes, programs),
            "mrr",
            ACADEMY_KPI_TARGETS,
            direction="higher_is_better",
        ),
...
def _coach_athlete_attendance(sessions: Sequence[Session], checkins: Sequence[CheckIn]) -> float:
    if not sessions:
        return 0.0
    attended = len(
        {
...
def compute_coach_metrics(
    coach: Coach, sessions: Sequence[Session], checkins: Sequence[CheckIn]
) -> Dict[str, Any]:
    ...
    delivered = sum(
        1 for s in coach_sessions if any(c.session_id == s.session_id for c in checkins)
    )
    ...
    return {
        ...
        "session_adherence": _with_target(
            session_adherence, "session_adherence", COACH_KPI_TARGETS, direction="higher_is_better"
        ),
        ...
        "session_delivery_ontime": _with_target(
            ontime, "session_delivery_ontime", COACH_KPI_TARGETS, direction="higher_is_better"
        ),
        "parent_satisfaction": _with_target(
            None, "parent_satisfaction", COACH_KPI_TARGETS, direction="higher_is_better"
        ),
    }


def compute_all_coach_metrics(
    coaches: Sequence[Coach], sessions: Sequence[Session], checkins: Sequence[CheckIn]
) -> Dict[str, Dict[str, Any]]:
    return {c.coach_id: compute_coach_metrics(c, sessions, checkins) for c in coaches}


def _with_target(
    value: Optional[float], kpi_id: str, targets: Dict[str, float], *, direction: str
) -> Dict[str, Any]:
    ...
    return {
        ...
        "met": (
            ...
            and (value >= target if direction == "higher_is_better" else value <= target)
        ),
    }
