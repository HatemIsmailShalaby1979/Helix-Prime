...
class AcademyCapabilityPack:
    def __init__(self, memory: GovernedMemory, *, phase: str = SUPERVISED,
        ...
        self.identity = identity or Identity(actor="academy-operator", actor_type="human",
    ...
    def validate_consent(self, consent: ConsentRecord, as_of: str) -> bool:
        ...
        self.mem.add(
            ...
            client_id=consent.client_id, actor="academy-pack", role_id="academy_admin",
            source="academy_consent", classification="client_confidential", timestamp=as_of,
    ...
    def exit_read_only_period(self, as_of: str, actor: str, role: str,
                              correlation_id: str = "academy-exit") -> str:
        ...
        self.mem.add(
            ...
            client_id="*pilot*", actor=actor, role_id=role, source="academy_phase",
    ...
    def prepare_first_real_pilot(self, starts_at: str, ends_at: str,
                                 consent: ConsentRecord, as_of: str):
    ...
    # ----------------------------------------------------------- connectors / diagnose
    def diagnose_account(self, tenant_id, client_id, as_of, operator_actor,
                         operator_role, correlation_id, fixtures):
        ...
        return {
            "ctx": ctx, "connectors": connectors, "diags": diags,
            "attendance": attendance, "churn": scores, "pipeline": pipe,
            "kpis": compute_academy_metrics(
                athletes=athletes, sessions=sessions, checkins=checkins,
                facility_slots=conn.list_facility_slots(ctx), programs=programs,
            ),
            "coach_kpis": compute_all_coach_metrics(
                conn.list_coaches(ctx), sessions, checkins),
            "facility": facility_overview(ctx, connectors),
            "fees": fee_status_overview(ctx, connectors, athletes, programs),
        }

    def _record_diagnosis_and_recommendations(self, tenant_id, client_id, as_of,
                                              operator_actor, operator_role,
                                              correlation_id, bundle):
        for d in bundle["diags"]:
            ev = list(d.evidence_refs)
            diag_rec = self.mem.add(
                kind="customer_context", nature="simulated_event",
                tenant_id=tenant_id, client_id=client_id,
                actor="academy-pack", role_id=operator_role, source="academy_diagnosis",
                classification="client_confidential", timestamp=as_of,
                correlation_id=correlation_id, confidence=d.confidence,
                evidence_refs=ev, data_mode=DATA_MODE,
                provenance={"correlation_id": correlation_id, "data_mode": DATA_MODE,
                            "basis": "academy_diagnosis", "sources": ev},
            ...
            for action in d.recommended_actions:
                rec = create_recommendation(
                    self.mem, tenant_id=tenant_id, client_id=client_id,
                    actor=operator_actor, role_id=operator_role,
                    correlation_id=correlation_id, timestamp=as_of,
                    action=action, evidence=ev, diagnosis_ref=diag_rec.record_id,
                    correct=False, data_mode=DATA_MODE,
                )
                create_approval_draft(
                    self.mem, tenant_id=tenant_id, client_id=client_id,
                    owner=operator_actor, role_id=operator_role,
                    correlation_id=correlation_id, timestamp=as_of,
                    action=action, recommendation_id=rec.record_id,
                    evidence=[rec.record_id], data_mode=DATA_MODE,
                )

    # ----------------------------------------------------------- dry run
    def dry_run(self, tenant_client_pairs, fixtures_map, as_of=DEFAULT_AS_OF,
                operator_actor="academy-operator", operator_role="academy_admin",
                correlation_id="academy-dryrun", consent: Optional[ConsentRecord] = None):
        ...
        for tenant_id, client_id in tenant_client_pairs:
            ...
            bundle = self.diagnose_account(
                tenant_id, client_id, as_of, operator_actor, operator_role,
                correlation_id, fixtures)
            last_bundle = bundle
            self.diagnoses.append((tenant_id, client_id, bundle["diags"]))
            record_attendance_outcome(
                self.mem, bundle["ctx"], bundle["attendance"], as_of,
                actor=operator_actor, role_id=operator_role)
            record_churn_flags(self.mem, bundle["ctx"], bundle["churn"], as_of,
                               actor=operator_actor, role_id=operator_role)
        ...
        self.mem.add(
            ...
            actor="academy-pack", role_id="academy_admin", source="academy_baseline",
            classification="internal", timestamp=as_of, correlation_id=correlation_id,
            confidence=1.0, evidence_refs=[], data_mode=DATA_MODE,
            provenance={"correlation_id": correlation_id, "data_mode": DATA_MODE,
    ...
    def approve_action(self, approval_id, approver_actor, approver_role,
                       requester_actor, requester_role, as_of=DEFAULT_AS_OF,
    ...
    def deny_action(self, approval_id, reviewer, reason, as_of=DEFAULT_AS_OF,
                    correlation_id="academy-deny"):
        prev = self._latest_approval(approval_id)
        if prev.kind != "approval":
            raise PilotError("not an approval record")
        return transition_approval(
    ...
    def rollback_action(self, approval_id, actor, role, reason, as_of=DEFAULT_AS_OF,
        ...
        self.mem.add(
            ...
            client_id=prev.client_id, actor=actor, role_id=role, source="academy_incident",
    ...
    def record_fee(self, ctx: ConnectorContext, *, athlete_id: str, family_id: str,
                   program_id: str, amount: float, currency: str, due_date: str,
                   paid_at, method_note: str, as_of: str,
                   actor: str = "academy-operator", role_id: str = "academy_admin"):
        ...
        return record_manual_payment(
            self.mem, ctx, athlete_id=athlete_id, family_id=family_id,
            program_id=program_id, amount=amount, currency=currency,
            due_date=due_date, paid_at=paid_at, method_note=method_note,
            as_of=as_of, actor=actor, role_id=role_id,
        )
    ...
    def generate_metacognitive_proposal(self, as_of: str, correlation_id: str,
        ...
        proposal = engine.propose(
            kind="workflow", target="attendance_ops",
            ...
            proposed_policy={"value": {"auto": False, "owner_role": "coach",
                                       "approver_role": "head_coach"}},
        ...
        engine.evaluate(
            proposal, historical_cases=[], simulated_cases=[{"low": True}, {"low": False}],
        ...
        self.mem.add(
            ...
            actor=actor, role_id=role, source="academy_metacognition",
