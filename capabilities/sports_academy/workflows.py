...
WORKFLOW_CATEGORIES = ("enrollment", "attendance_ops", "renewal",
                       "facility_booking", "fee_record")
...
def enrollment_flow(enrollment_pipeline: Dict[str, Any]) -> AcademyDiagnosis:
    ...
    if stages.get("trial", 0) > 0:
        findings.append(RiskFinding(
            "enrollment", "high" if stages["trial"] > 1 else "medium",
    ...
    state = "critical" if any(f.severity == "high" for f in findings) else (
        "at_risk" if findings else "ok")
    return AcademyDiagnosis(
        "enrollment", state, 0.85 if findings else 1.0, tuple(findings),
        actions, tuple(f.evidence_ref for f in findings),
    )


def attendance_flow(sessions: Sequence[Any], checkins: Sequence[Any],
                    as_of: str) -> AcademyDiagnosis:
    """Daily check-in → adherence report: flag low-attendance sessions."""
    att = compute_attendance(sessions, checkins)
    low = [(sid, row) for sid, row in att["by_session"].items()
           if row["roster_size"] and row["attendance_rate"] < LOW_SESSION_ATTENDANCE]
    findings = [
        RiskFinding("attendance_ops", "high" if row["attendance_rate"] < 0.4 else "medium",
    ...
    state = "critical" if any(f.severity == "high" for f in findings) else (
        "at_risk" if findings else "ok")
    return AcademyDiagnosis(
        "attendance_ops", state, 0.9 if findings else 1.0, tuple(findings),
        actions, tuple(f.evidence_ref for f in findings),
    )


def renewal_flow(athletes: Sequence[Any], fee_payments: Sequence[Any],
                 as_of: str) -> AcademyDiagnosis:
    ...
    actions = tuple(
        ...
        f"Schedule retention conversation for {aid} before term end"
        for aid in at_risk
    )
    state = "critical" if any(f.severity == "high" for f in findings) else (
        "at_risk" if findings else "ok")
    return AcademyDiagnosis(
        "renewal", state, 0.8 if findings else 1.0, tuple(findings),
        actions, tuple(f.evidence_ref for f in findings),
    )


def churn_risk_signals_for(athletes: Sequence[Any],
                           fee_payments: Sequence[Any]) -> Sequence[Dict[str, Any]]:
    ...
    return [
        {"athlete_id": p.athlete_id, "reason": "outstanding_fee",
