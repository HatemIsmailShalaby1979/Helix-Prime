...
def _src(
    provider: str, record_id: str, observed_at: str, version: str, data_mode: str
) -> SourceRef:
...
def _ticket(
    ...
    *,
    ...
    return SupportTicket(
        ...
        sla_breached,
        observed_at,
        _src("Zendesk", ticket_id, observed_at, "zd-v1", data_mode),
        ctx.tenant_id,
        ctx.client_id,
    )
...
def healthy_account(ctx, data_mode="simulated_realistic", as_of=DEFAULT_AS_OF, stale=False):
    ...
    t = _ticket(
        ctx, "tk-h1", "acct-healthy", _offset(as_of, 3 if not stale else STALE_DAYS), data_mode
    )
    e = _enrichment(
        ctx,
        "acct-healthy",
        _offset(as_of, 2 if not stale else STALE_DAYS),
        data_mode,
...
def at_risk_account(ctx, data_mode="simulated_realistic", as_of=DEFAULT_AS_OF, stale=False):
    ...
    t = _ticket(
        ...
        "tk-r1",
        "acct-risk",
        _offset(as_of, 1 if not stale else STALE_DAYS),
        data_mode,
        sla_breached=True,
        priority="high",
        status="open",
