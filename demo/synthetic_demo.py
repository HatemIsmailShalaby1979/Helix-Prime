...
def _consent(tenant_id, client_id):
    return ConsentRecord(
        consent_id=f"consent-{tenant_id}",
        tenant_id=tenant_id,
        client_id=client_id,
        customer_id=f"cust-{tenant_id}",
        status="granted",
...
def main() -> int:
    ...
    cc.prepare_first_real_pilot(
        "2026-08-01T00:00:00Z", "2026-09-01T00:00:00Z", _consent("cc-1", "cc-c1"), AS_OF
    )
    ...
    rt.prepare_first_real_pilot(
        "2026-08-01T00:00:00Z", "2026-09-01T00:00:00Z", _consent("r1", "rc1"), AS_OF
    )
    ...
    ctx = ConnectorContext(
        "r1", "org-1", "rc1", actor="x", correlation_id="c", data_mode="simulated_realistic"
    )
    from capabilities.restaurant.contracts import RestaurantConnector

    assert (
        RestaurantConnector("restaurant_ops", "RestaurantOps", {})
        .request_write(ctx, "reorder", {}, None)
        .executed
        is False
    )
    ...
    print(
        "[call-centre pilot] live_customer_records:",
        cc_pack["live_customer_records"],
        "audit_chain_intact:",
        cc_pack["audit_chain_intact"],
    )
    ...
    print(
        "[restaurant pack] live_customer_records:",
        rt_pack["live_customer_records"],
        "audit_chain_intact:",
        rt_pack["audit_chain_intact"],
    )
