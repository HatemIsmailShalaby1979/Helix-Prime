    ...
    try:
        ...
        for i, c in enumerate(customers):
            ...
            if not any(k in c for k in ("csat", "sla", "fcr", "aht", "churn_risk")):
                raise ValueError(
                    f"customers[{i}] must have at least one of csat/sla/fcr/aht/churn_risk"
                )
            # Check out-of-range
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                result = type(
                    ...
                    (),
                    {
                        "customer_risks": scored,
                        "overall_risk_score": sum(s.get("csat", 0) for s in scored) / len(scored)
                        if scored
                        else 0,
                    },
                )()
        ...
        if metrics.get("high_risk_customers"):
            recommendations.append(
                {"type": "churn", "action": "review high risk customers", "source": "model"}
            )
