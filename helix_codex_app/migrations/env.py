"""Alembic environment for the Helix Codex App SQLite schema.
...
2. ``HELIX_APP_DB_PATH`` environment (same variable config.py reads)
3. ``helix_codex_app/app.db``       (local-first default)
...
DEFAULT_DB_PATH = "helix_codex_app/app.db"


def _resolve_db_url() -> str:
    x_args = context.get_x_argument(as_dictionary=True)
    db_path = x_args.get("db") or os.environ.get("HELIX_APP_DB_PATH") or DEFAULT_DB_PATH
