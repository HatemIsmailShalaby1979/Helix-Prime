#!/usr/bin/env python3
"""
App migration drift check (P1.1, Prompt 5).

The app schema has two sources of truth:
  1. helix_codex_app/db.py                — what the application creates at runtime
  2. helix_codex_app/migrations/versions/ — the authoritative schema history

This script proves they agree, empirically: it builds one database via db.py
and one via ``alembic upgrade head``, then compares their full sqlite_master
contents (tables — names and DDL text). Any divergence in either direction
fails CI with a per-object report.

Alembic's own bookkeeping (``alembic_version``) is excluded: it exists only in
the migrated database and carries no schema meaning.
...
def _build_store_schema(db_path: str) -> dict[str, str]:
    from helix_codex_app.db import _init_schema, close, connect
...
def check_drift() -> tuple[list[str], dict[str, object]]:
    with tempfile.TemporaryDirectory(prefix="helix_app_migration_drift_") as work:
        ...
        for name in missing_from_migrations:
            errors.append(
                f"{name}: created by helix_codex_app/db.py but absent from the migration head"
            )
        for name in missing_from_store:
            errors.append(
                f"{name}: present in the migration head but helix_codex_app/db.py does not create it"
            )
        for name in altered:
            errors.append(
                f"{name}: DDL differs between helix_codex_app/db.py and the migration head"
                f" (db.py: {store_schema[name]!r} vs migration: {migrated_schema[name]!r})"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            f"App migration drift check: db.py objects={report['store_objects']}, "
        ...
        if errors:
            ...
            print("No drift: helix_codex_app/db.py and helix_codex_app/migrations/ head agree.")
