...
def load_role_catalog(path: str | pathlib.Path = _CATALOG_PATH) -> Dict[str, Any]:
    ...
    if yaml is None:
        raise ValueError(
            "PyYAML not installed: cannot load organization/role-catalog.yaml (pip install pyyaml)"
        )
...
def validate_role_catalog(
    data: Dict[str, Any], source_path: str = "role-catalog.yaml"
) -> Dict[str, Any]:
    """
    ...
    if kpi_vocab is not None:
        if not isinstance(kpi_vocab, list):
            raise ValueError(
                f"{source_path}.kpi_vocabulary: must be list, got {type(kpi_vocab).__name__}"
            )
        kpi_vocab = _require_list_of_str(
            kpi_vocab, f"{source_path}.kpi_vocabulary", allow_empty=False
        )
    else:
        kpi_vocab = []
    ...
    for idx, role in enumerate(roles):
        if not isinstance(role, dict):
            raise ValueError(
                f"{source_path}.roles[{idx}]: must be mapping, got {type(role).__name__}"
            )
        ...
        if rid in seen:
            raise ValueError(
                f"{source_path}: duplicate role id {rid!r} at {role_path} and roles[{seen[rid]}]"
            )
        seen[rid] = idx
        ...
        if "implementation_status" in role:
            status = _require_non_empty_str(
                role["implementation_status"], f"{role_path}.implementation_status"
            )
        ...
        _require_list_of_str(
            role["owned_capabilities"], f"{role_path}.owned_capabilities", allow_empty=False
        )
        _require_list_of_str(role["allowed_tools"], f"{role_path}.allowed_tools", allow_empty=False)
        _require_list_of_str(
            role["readable_data_domains"], f"{role_path}.readable_data_domains", allow_empty=False
        )
        _require_list_of_str(role["kpis"], f"{role_path}.kpis", allow_empty=False)
        _require_list_of_str(
            role["allowed_peer_calls"], f"{role_path}.allowed_peer_calls", allow_empty=True
        )
        ...
        if kpi_vocab:
            for kpi in role["kpis"]:
                if kpi not in kpi_vocab:
                    raise ValueError(f"{role_path}.kpis: {kpi!r} not in kpi_vocabulary {kpi_vocab}")

        # approval_limits
        al = role["approval_limits"]
        if not isinstance(al, dict):
            raise ValueError(
                f"{role_path}.approval_limits: must be mapping, got {type(al).__name__}"
            )
        ...
        _require_list_of_str(
            al["can_approve"], f"{role_path}.approval_limits.can_approve", allow_empty=True
        )
        ...
        if isinstance(mfa, int) and mfa < 0:
            raise ValueError(
                f"{role_path}.approval_limits.max_financial_amount: must be >=0, got {mfa}"
            )
        _require_list_of_str(
        ...
        if not isinstance(sod, dict):
            raise ValueError(
                f"{role_path}.segregation_of_duties: must be mapping, got {type(sod).__name__}"
            )
        ...
        _require_list_of_str(
            sod["must_be_reviewed_by"],
            f"{role_path}.segregation_of_duties.must_be_reviewed_by",
            allow_empty=True,
        )
        _require_list_of_str(
    ...
    if missing:
        raise ValueError(
            f"{source_path}: missing required role ids {sorted(missing)} (need SAMI + 8 GMs)"
        )
    ...
    for rid, role in roles_by_id.items():
        ...
        if esc not in all_ids:
            raise ValueError(
                f"{role_path}.escalation_owner: {esc!r} not in role ids {sorted(all_ids)}"
            )
        for peer in role["allowed_peer_calls"]:
            if peer not in all_ids:
                raise ValueError(
                    f"{role_path}.allowed_peer_calls: {peer!r} not in role ids {sorted(all_ids)}"
                )
            if peer == rid:
        ...
        for reviewer in sod["must_be_reviewed_by"]:
            if reviewer not in all_ids:
                raise ValueError(
                    f"{role_path}.segregation_of_duties.must_be_reviewed_by: {reviewer!r} not in role ids"
                )
...
def requires_compliance_review(catalog: Dict[str, Any], role_id: str) -> bool:
    ...
    return "compliance_quality_gm" in role.get("segregation_of_duties", {}).get(
        "must_be_reviewed_by", []
    )
