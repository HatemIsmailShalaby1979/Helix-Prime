...
def _fixtures():
    return {
        (("r1", "rc1")): build_synthetic_restaurant("r1", "rc1", TS),
...
def test_approval_gating():
    ...
    try:
        rt.approve_action(
            draft.record_id, "approver-1", "restaurant_gm", owner, owner_role, as_of=TS
        )
    except Exception:
        blocked = True
    assert blocked
    ...
    try:
        rt.approve_action(
            draft.record_id, "approver-1", "shift_manager", owner, owner_role, as_of=TS
        )
    except Exception:
        wrong = True
    assert wrong
...
def test_failure_handling():
    ...
    rt._build_connectors = lambda ctx, fixtures: {
        "restaurant_ops": BrokenConnector("restaurant_ops", "RestaurantOps", fixtures)
    }
    rt.dry_run([("r1", "rc1")], _fixtures(), consent=_valid_consent())
    fails = [
        r
        for r in rt.mem._records
        if r.kind == "workflow_history" and r.body.get("action") == "connector_failure"
    ]
    assert fails
...
def test_metacognitive_proposal_reuse():
    ...
    pol = [
        r
        for r in rt.mem._records
        if r.kind == "policy" and r.body.get("proposal_id") == report["proposal_id"]
    ]
...
def test_connector_read_only_contract():
    ctx = ConnectorContext("r1", "org-1", "rc1", actor="x", correlation_id="c", data_mode=DATA_MODE)
    conn = RestaurantConnector(
        "restaurant_ops", "RestaurantOps", build_synthetic_restaurant("r1", "rc1", TS)
    )
